...
def prediction():
    ...
    # 3. Prepare input and predict
    query = np.array([temp, humidity, ph, rainfall, season, state, nitrogen, phosphorous, potassium]).reshape(1, -1)
    result = model.predict(query)[0]  # Predict top crop (instead of predict_proba)

    # 4. Fertilizer recommendation for predicted crop
    df = pd.read_csv("fertilizer.csv")  # Path to your fertilizer dataset
    nr = df[df['Crop'] == result]['N'].iloc[0]
    pr = df[df['Crop'] == result]['P'].iloc[0]
    kr = df[df['Crop'] == result]['K'].iloc[0]

    n_diff = nr - nitrogen
    p_diff = pr - phosphorous
    k_diff = kr - potassium

    temp_dic = {abs(n_diff): "N", abs(p_diff): "P", abs(k_diff): "K"}
    max_key = temp_dic[max(temp_dic.keys())]
    ...
    fertilizer_advice = fertilizer_dic[key]

    return render_template('Crop_data.html', crop=result, fertilizer=fertilizer_advice)
